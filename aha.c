...
main(int argc, char *argv[])
{
   ...
   for (i = 0; i < NTRIALX; i++) {
      ...
      printf("userfun(0x%X) = 0x%X\n", trialx[i], correct_result[i]);
#elif NARGS == 2
      for (j = 0; j < NTRIALY; j++)
         correct_result[i][j] = userfun(trialx[i], trialy[j]);
         printf("userfun(0x%X, 0x%Xd) = 0x%X\n", trialx[i], trialy[i], correct_result[i]);
#elif NARGS == 3
      for (j = 0; j < NTRIALY; j++)
         for (k = 0; k < NTRIALZ; k++)
            correct_result[i][j][k] = userfun(trialx[i], trialy[j], trialz[k]);
            printf("userfun(0x%X, 0x%X, 0x%X) = 0x%X\n", trialx[i], trialy[i], trialz[i], correct_result[i]);
#endif
   }
